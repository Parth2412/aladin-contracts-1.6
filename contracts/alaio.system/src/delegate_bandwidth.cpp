...
namespace alaiosystem {
   ...
   void system_contract::buyrambytes( name payer, name receiver, uint32_t bytes ) {
      ...
      auto tmp = *itr;
      auto alaout = tmp.convert( asset(bytes, ram_symbol), core_symbol() );

      buyram( payer, receiver, alaout );
   }
  ...
  /**
                                   ...
                                   const asset stake_net_delta, const asset stake_cpu_delta, bool transfer )
   {
      ...
      if ( stake_account != source_stake_from ) { //for alaio both transfer and refund make no sense
                                       ...
                                       asset unstake_net_quantity, asset unstake_cpu_quantity )
   {
      ...
      auto producer_from_name = _voters.find(from.value);
      if(producer_from_name -> staked > 1000000){
