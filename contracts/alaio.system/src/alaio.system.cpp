...
#include <alaiolib/dispatcher.hpp>
...
namespace alaiosystem {
    ...
    _oracles( get_self(), get_self().value )
   {
      ...
      _gstate2 = _global2.exists() ? _global2.get() : alaio_global_state2{};
      _gstate3 = _global3.exists() ? _global3.get() : alaio_global_state3{};
      _gstate4 = _global4.exists() ? _global4.get() : alaio_global_state4{};
      _oracle_state = _oracle_global.get_or_create(get_self(), oracle_reward_info{});      
   }

   alaio_global_state system_contract::get_default_parameters() {
      alaio_global_state dp;
   ...
   void system_contract::setparams( const alaio::blockchain_parameters& params ) {
      require_auth( _self );
      (alaio::blockchain_parameters&)(_gstate) = params;
   ...
   void native::setabi( name acnt, const std::vector<char>& abi ) {
      alaio::multi_index< "abihash"_n, abi_hash >  table(_self, _self.value);
   ...
   void system_contract::init( unsigned_int version, symbol core ) {
      ...
      auto system_token_supply   = alaio::token::get_supply(token_account, core.code() );
      ...
      INLINE_ACTION_SENDER(alaio::token, open)( token_account, { _self, active_permission },
...
ALAIO_DISPATCH( alaiosystem::system_contract,
     // native.hpp (newaccount definition is actually in alaio.system.cpp)
     ...
     // producer_pay.cpp
     (onblock)(claimrewards)(claimdapprwd)(claimvoterwd)(setvclaimprd)
     //oracle.cpp
     (addrequest)(reply)(setoracle)
)
