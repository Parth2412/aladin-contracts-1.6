...
#include <alaio.system/rex.results.hpp>
...
namespace alaiosystem {
   ...
   void system_contract::buyrex( const name& from, const asset& amount )
   {
      ...
      rex_results::buyresult_action buyrex_act( rex_account, std::vector<alaio::permission_level>{ } );
   ...
   void system_contract::sellrex( const name& from, const asset& rex )
   {
      ...
      if ( current_order.success ) {
         rex_results::sellresult_action sellrex_act( rex_account, std::vector<alaio::permission_level>{ } );
   ...
   void system_contract::runrex( uint16_t max )
   {
      ...
      /// transfer from alaio.names to alaio.rex
         ...
         for ( uint16_t i = 0; i < max; ++i ) {
            ...
            if ( bitr != _rexbalance.end() ) { // should always be true
               ...
               if ( result.success ) {
                  ...
                  rex_results::orderresult_action order_act( rex_account, std::vector<alaio::permission_level>{ } );
   ...
   int64_t system_contract::rent_rex( T& table, const name& from, const name& receiver, const asset& payment, const asset& fund )
   {
      ...
      table.emplace( from, [&]( auto& c ) {
         ...
         c.expiration   = current_time_point() + alaio::days(30);
         c.loan_num     = pool->loan_num;
      });

      rex_results::rentresult_action rentresult_act{ rex_account, std::vector<alaio::permission_level>{ } };
   ...
   void system_contract::channel_to_rex( const name& from, const asset& amount )
   {
      ...
      if ( rex_available() ) {
         ...
         transfer_act.send( from, rex_account, amount,
                            std::string("transfer from ") + from.to_string() + " to alaio.rex" );
      }
#endif
   }
   ...
   {
      /**
       * If CORE_SYMBOL is (ALA,4), maximum supply is 10^10 tokens (10 billion tokens), i.e., maximum amount
