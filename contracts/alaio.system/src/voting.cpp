...
#include <alaiolib/datastream.hpp>
#include <alaiolib/serialize.hpp>
#include <alaiolib/multi_index.hpp>
...
namespace alaiosystem {
   ...
   using alaio::singleton;
   using alaio::transaction;
    ...
    *
    */
   void system_contract::regproducer( const name producer, const alaio::public_key& producer_key, const std::string& url, uint16_t location ) {
      check( url.size() < 512, "url too long" );
      check( producer_key != alaio::public_key(), "public key should not be the default value" );
      require_auth( producer );
      auto producer_name = _voters.find(producer.value);
      check(producer_name -> staked > 1000000000, "You have 100'000 ALA tokens to become a Block Producer");
   ...
   void system_contract::update_elected_producers( block_timestamp block_time ) {
      ...
      std::vector< std::pair<alaio::producer_key,uint16_t> > top_producers;
      top_producers.reserve(21);

      for ( auto it = idx.cbegin(); it != idx.cend() && top_producers.size() < 21 && 0 < it->total_votes && it->active(); ++it ) {
         top_producers.emplace_back( std::pair<alaio::producer_key,uint16_t>({{it->owner, it->producer_key}, it->location}) );
      }
      ...
      std::vector<alaio::producer_key> producers;
   ...
   double system_contract::update_total_witnesspay_share( time_point ct, double additional_shares_delta, double shares_rate_delta ){
      ...
      alaio::print("=== shares_rate_delta ===",shares_rate_delta);
   ...
   void system_contract::update_votes( const name voter_name, const name proxy, const std::vector<name>& producers, bool voting ) {
      ...
      for ( auto it = idx.cbegin(); it != idx.cend() && witnesses_before.size() < max_witness_count && (it->total_votes / _gstate.total_producer_vote_weight) > 0.005 && it->active(); ++it ) {
         ...
         alaio::print("+++ witnesses_before: ",it->owner);
      }
      ...
      for ( auto it = idx.cbegin(); it != idx.cend() && witnesses_after.size() < max_witness_count && (it->total_votes / _gstate.total_producer_vote_weight) > 0.005 && it->active(); ++it ) {
         ...
         alaio::print("=== witnesses_after",it->owner);
      }
      ...
      // alaio::print("ppppppppppppppppppppp",witnesses_before);
      
      // for(auto& prod: witnesses_after){
      //    alaio::print("aaaaaaaaaaaaaaaaaaaaa",prod);
      // }
      // for(auto& prod2: witnesses_before){
      //    alaio::print("bbbbbbbbbbbbbbbbbbbbbb",prod2);
      // }
