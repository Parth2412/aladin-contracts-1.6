#include <alaio.msig/alaio.msig.hpp>
#include <alaiolib/action.hpp>
#include <alaiolib/permission.hpp>
#include <alaiolib/crypto.hpp>
                        ...
                        ignore<transaction> trx )
{
   ...
   check( _trx_header.expiration >= alaio::time_point_sec(current_time_point()), "transaction expired" );
...
void multisig::cancel( name proposer, name proposal_name, name canceler ) {
   ...
   if( canceler != proposer ) {
      check( unpack<transaction_header>( prop.packed_transaction ).expiration < alaio::time_point_sec(current_time_point()), "cannot cancel until expiration" );
   }
...
void multisig::exec( name proposer, name proposal_name, name executer ) {
   ...
   ds >> trx_header;
   check( trx_header.expiration >= alaio::time_point_sec(current_time_point()), "transaction expired" );
...
ALAIO_DISPATCH( alaio::multisig, (propose)(approve)(unapprove)(cancel)(exec)(invalidate) )
