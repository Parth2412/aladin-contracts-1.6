...
namespace eosiosystem {
   ...
   double stake2vote( int64_t staked ) {
      ...
      // Any user cannot vote more than 1000 at a time
      // int64_t upperbound = 1000 * std::pow(10, core_symbol().precision());
      // return double((staked >= upperbound) ? upperbound : staked);

      // double weight = int64_t( (now() - (block_timestamp::block_timestamp_epoch / 1000)) / (seconds_per_day * 7) )  / double( 52 );
      // if(staked >= 10000000){
      //    return 10000000 * std::pow( 2, weight );
      // }
      // else{
      //    return double(staked) * std::pow( 2, weight );
      // }
      
      return double((staked >= 10000000) ? 10000000 : staked);
   }
   ...
   void system_contract::check_voter( const name& owner, const char* error_msg )const
   {
      auto pitr = _producers.find(owner.value);
      check(pitr == _producers.end(), error_msg);
      
   }
   
   void system_contract::voteproducer( const name voter_name, const name proxy, const std::vector<name>& producers ) {
      check_voter(voter_name);
      require_auth( voter_name );
      vote_stake_updater( voter_name );
      update_votes( voter_name, proxy, producers, true );
      auto rex_itr = _rexbalance.find( voter_name.value );
      if( rex_itr != _rexbalance.end() && rex_itr->rex_balance.amount > 0 ) {
         check_voting_requirement( voter_name, "voter holding REX tokens must vote for at least 7 producers or for a proxy" );
      }
   }
